...
interface VR {
  ...
  final_result_status: "effective" | "ineffective" | "interfered" | "insufficient";
  notes: string | null;
...
const STATUS_CFG: Record<string, { icon: React.ComponentType<{ size?: number; className?: string }>; color: string; label: string; bg: string }> = {
  ...
  insufficient:  { icon: HelpCircle,    color: "text-[#86868b]", label: "数据不足", bg: "bg-[#f5f5f7]" },
};

export default function ValidationResults() {
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery<{ items: VR[]; total: number }>({
    queryKey: ["validation-results"],
    queryFn: async () => {
      const r = await fetch(`${API}/validation-results?page_size=50`);
      return r.json();
    },
  });
  ...
  const [form, setForm] = useState({ asin: "", status: "effective", notes: "", attribution: "" });
  ...
  const counts = {
    ...
    insufficient: results.filter((r) => r.final_result_status === "insufficient").length,
  };

  const handleSubmit = async () => {
    if (!form.asin.trim()) return;
    setSubmitting(true);
    try {
      ...
      setFormOpen(false);
      setForm({ asin: "", status: "effective", notes: "", attribution: "" });
  ...
  return (
    <div className="max-w-[800px] mx-auto py-8">
      <div className="mb-8">
        ...
        <p className="text-[17px] text-[#86868b]">前后指标对比，判断有效 / 无效 / 受干扰，决定继续 / 调整 / 停止</p>
      </div>
        ...
        <div className="apple-card p-5 mb-6 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.asin} onChange={e => setForm({ ...form, asin: e.target.value })} placeholder="B0XXXXXXXX" className="apple-input" />
            </div>
            <div>
          ...
          <div>
            ...
            <input value={form.attribution} onChange={e => setForm({ ...form, attribution: e.target.value })} className="apple-input" placeholder="什么导致了有效/无效" />
          </div>
          <div className="flex gap-2">
            <button onClick={handleSubmit} disabled={submitting || !form.asin.trim()} className="apple-btn-primary px-5 py-2 text-[14px]">
              {submitting ? "提交中..." : "提交"}
            </button>
      ...
      {results.length === 0 ? (
        ...
        <div className="space-y-3">
          {results.map((r) => {
            const cfg = STATUS_CFG[r.final_result_status] ?? STATUS_CFG.insufficient;
            const Icon = cfg.icon;
              ...
              <div key={r.id} className={`apple-card border-l-4 ${r.final_result_status === "effective" ? "border-l-[#34c759]" : r.final_result_status === "ineffective" ? "border-l-[#ff3b30]" : r.final_result_status === "interfered" ? "border-l-[#ff9500]" : "border-l-[#86868b]"}`}>
                <div className="p-4 flex items-center gap-3 cursor-pointer" onClick={() => setExpanded(open ? null : r.id)}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[13px] text-[#86868b] truncate">{r.attribution_conclusion || "无归因结论"}</p>
                  </div>
                ...
                {open && (
                  <div className="px-4 pb-4 border-t border-[#d2d2d7]/20 pt-3">
                    <p className="text-[14px]">{r.notes || "无备注"}</p>
                    ...
                    {r.before_metrics && r.after_metrics && (
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        {Object.keys(r.before_metrics).map((k) => {
                          const before = r.before_metrics?.[k] ?? 0;
                          const after = r.after_metrics?.[k] ?? 0;
