...
本次不是把8D+2当作10个平铺同级指标简单平均。必须先用两把尺判断，再把8D+2作为反向检查视角：
...
### 8D+2归位规则
- 用户意图层：功能表达、场景表达、身份适配、心理利益、风险消除、主观属性。
- 平台对齐层：产品身份、兼容搭配、场景表达、身份适配、功能表达。
- Listing承接层：差异化、风险消除、主观属性、功能表达、兼容搭配。
- 辅助验证层：市场趋势。趋势只用于验证和排序，不能替代两把尺主判断。

每个analysis字段必须说明：维度归属、两把尺映射、当前证据、扣分原因、问题类型（对齐/缺失/错位/断链/误导）、影响指标（CTR/CVR/CPC/ACOS/退货/差评）、下一步动作。
...
async def _build_listing_evidence_chain(listing: ListingInput, ai_service: AIHubService) -> dict:
    ...
    evidence: dict = {
        ...
        "task_boundaries": {
            ...
            "AI_DEEP_MODEL": "只做综合诊断：必须引用向量、精排、OCR事实和抓取字段，先按两把尺判断用户意图与平台理解，再用8D+2反向检查承接、关系词、状态词和广告验证假设。",
...
async def _diagnose_single(
    ...
    system_msg = (
        ...
        f' 7.本次诊断采用“两把尺 × 8D+2反向检查”：先判断用户意图标准和平台理解标准，再把8D+2归入用户意图层、平台对齐层、Listing承接层、辅助验证层；禁止把10项当作平铺平均分。'
