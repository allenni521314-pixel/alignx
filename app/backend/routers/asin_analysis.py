...
本任务不是把10个维度当作平铺同级指标简单平均。必须先判断竞品/ASIN背后的趋利/避害、人性节点、动机、需求、场景和解决方案，再判断为什么被用户选择、为什么被Amazon匹配，最后用10维诊断做反向检查。输出字段值中禁止出现“人性根层、趋利、避害、Level、13个人性节点、后台判断、内部方法论”等后台链路词。
...
每个analysis字段必须输出：用户需求映射、平台识别映射、真实证据、强点/漏洞、我方动作（借鉴/避开/攻击/差异化）和广告验证假设。后台判断必须按1人性根层、2用户意图、3平台规则、4验证回流的顺序执行，但不要把内部流程原文暴露给前台卖家。
